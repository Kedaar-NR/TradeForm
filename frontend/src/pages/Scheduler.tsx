...
const Scheduler: React.FC = () => {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      console.error("Failed to parse CAD/BOM:", err);
      setError("Could not parse that file. Try another .xlsx or .csv.");
    } finally {
      setIsParsing(false);
    }
  };
  ...
  const summary = useMemo(() => {
    ...
    const totalBuildTime = items.reduce((sum, item) => sum + item.buildTimeDays, 0);
      ...
      <div className="card p-6 border-dashed border-2 border-gray-200 bg-gradient-to-br from-white to-gray-50">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            <h3 className="text-lg font-semibold text-gray-900">
              Upload CAD Design (.xlsx / .csv)
            </h3>
            <p className="text-sm text-gray-600">
              Upload your design file, then click Parse to generate a bill of materials. We look for columns like &ldquo;Component&rdquo;, &ldquo;Qty&rdquo;, &ldquo;Lead Time&rdquo;, and &ldquo;Build Time&rdquo;.
            </p>
          </div>
        ...
        {fileName && !isParsing && items.length === 0 && (
          <div className="mt-3 flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-700 font-medium">File ready: {fileName} — Click Parse to generate BOM</span>
          </div>
        )}
        {isParsing && (
          <div className="mt-3 flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-700 font-medium">Parsing {fileName}...</span>
          </div>
        )}
        {items.length > 0 && (
          <div className="mt-3 flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-700 font-medium">BOM generated from: {fileName}</span>
          </div>
        )}
        {error && (
      ...
      {items.length > 0 ? (
        ...
        <div className="card p-10 text-center">
          ...
          <p className="text-sm text-gray-600 max-w-xl mx-auto">
            Upload an Excel or CSV BOM to see procurement + build timing per line item.
            Use the defaults above if your sheet doesn’t contain lead/build columns.
          </p>
        </div>
      )}
    </div>
