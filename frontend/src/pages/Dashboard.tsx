...
import { ProjectCard, StudyCard, CreateProjectModal, MoveStudyContextMenu } from "../components/Dashboard";
import type { ApiProjectGroup } from "../types/api";

// Known template study names for filtering
const TEMPLATE_STUDY_NAMES = new Set(
    [
        "Booster Selection", "Payload Fairing", "Avionics Package", "Fuel Tank Selection",
        "Thrust Vector Control", "Ignition System", "Turbopump Selection", "Recovery System",
        "Nozzle Design", "Guidance Sensor Suite", "Separation System", "Solar Panel Selection",
        "Communication Antenna", "Reaction Wheel", "Battery System", "Star Tracker",
        "Onboard Computer", "Propulsion System", "Thermal Control", "GPS Receiver",
        "Payload Instrument", "Motor Selection", "Camera Module", "Battery Pack",
        "Propeller Selection", "Flight Controller", "ESC Selection", "GPS Module",
        "Gimbal System", "FPV System", "Turbine Engine Selection", "Flight Control Computer",
        "Composite Wing Spar", "Avionics Display", "Structural Component",
    ].map((n) => n.toLowerCase())
);
...
const Dashboard: React.FC = () => {
    ...
    const loadProjectGroups = useCallback(async () => {
        try {
            ...
            setProjectGroups(transformProjectGroups(response.data as unknown as ApiProjectGroup[]));
        } catch (error) {
    ...
    const loadProjects = useCallback(async () => {
        try {
            ...
            transformedProjects.forEach((proj: { projectGroupId?: string; name: string; id: string; createdViaTemplateGroup?: boolean }) => {
                if (!proj.projectGroupId && TEMPLATE_STUDY_NAMES.has(proj.name.toLowerCase())) {
    ...
    const handleCreateProject = async () => {
        ...
        try {
            ...
            const newGroup: ProjectGroup = {
                ...
                createdAt: (response.data as { created_at?: string }).created_at || new Date().toISOString(),
                updatedAt: (response.data as { updated_at?: string }).updated_at || new Date().toISOString(),
    ...
    const handleDeleteStudy = async (studyId: string, studyName: string) => {
        if (!window.confirm(`Delete "${studyName}"? This cannot be undone.`)) return;
        try {
                        ...
                        <div className="space-y-3">
                            {filteredStudies.map((study) => (
                                <StudyCard
                                    ...
                                    onMoveClick={(e, id) => {
                                        e.stopPropagation();
                                        setContextMenu({ projectId: id, x: e.clientX, y: e.clientY });
                                    }}
                                    onContextMenu={(e, id) => {
                                        e.preventDefault();
                                        setContextMenu({ projectId: id, x: e.clientX, y: e.clientY });
