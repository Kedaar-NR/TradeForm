...
import { BackButton, ProgressStepper, TRADE_STUDY_STEPS } from "../components/ui";
import { ResultsTable, ResultsHeatmap, ResultsCharts, ComponentDetailModal } from "../components/Results";
...
const Results: React.FC = () => {
    ...
    const loadResults = useCallback(async () => {
        ...
        try {
            ...
            const criteriaData = transformCriteria(criteriaRes.data as unknown as ApiCriterion[], projectId);

            // Transform results to match our interface
            const transformedComponents: ComponentScore[] = resultsData.results.map(
                (result: { component: { id: string; manufacturer: string; part_number: string }; scores: Array<{ criterion_id: string; score: number; rationale: string }>; total_score: number; rank: number }) => {
                    const critMap: Record<string, { score: number; rationale: string; weight: number }> = {};

                    criteriaData.forEach((criterion) => {
                        const score = result.scores.find((s) => s.criterion_id === criterion.id);
    ...
    const handleExportCSV = useCallback(() => {
        ...
        downloadBlob(new Blob([csvLines], { type: MIME_TYPES.CSV }), `trade-study-results-${dateSlug}.csv`);
    }, [components]);
    ...
    const handleExportFullExcel = useCallback(() => {
        ...
        try {
            ...
            const detailedHeaders = ["Rank", "Manufacturer", "Part Number", "Criterion", "Score", "Weight", "Rationale"];
            ...
            const dateSlug = formatDateForFilename(new Date());
            downloadBlob(new Blob([excelBuffer], { type: MIME_TYPES.XLSX }), `TradeStudy_Full_${dateSlug}.xlsx`);
        } catch (error) {
    ...
    const handleExportWord = useCallback(async () => {
        ...
        try {
            ...
            alert(`Failed to export Word: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }, [projectId]);

    const winner = useMemo(() => (components.length > 0 ? components[0] : null), [components]);
        ...
        return (
            <div className="max-w-6xl animate-fade-in">
                <div className="card p-12 text-center">
                    ...
                    <p className="text-sm text-gray-600 mb-6">Add components and scores to see results</p>
        ...
        <div className="max-w-6xl animate-fade-in">
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Trade Study Results</h1>
                        <p className="text-gray-600">AI-scored components ranked by weighted criteria</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={handleExportFullExcel} className="btn-primary flex items-center gap-2">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            Export Full Report (Excel)
                        </button>
                        <button onClick={handleExportWord} className="btn-secondary flex items-center gap-2">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            Export Report (Word)
            ...
            {winner && (
                <div className="bg-gray-100 border border-gray-300 rounded-lg p-6 mb-8">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <div className="text-xs font-medium text-gray-900 mb-1">RECOMMENDED</div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-1">
                                {winner.manufacturer} {winner.partNumber}
                            </h3>
                            <p className="text-sm text-gray-600">
                                Weighted Score:{" "}
                                <span className="font-semibold text-gray-900">{winner.totalScore.toFixed(2)}/10</span>
                            </p>
                        </div>
                        <button onClick={() => setSelectedComponent(winner)} className="btn-secondary whitespace-nowrap">
                            View Details
                        </button>
                    </div>
                </div>
