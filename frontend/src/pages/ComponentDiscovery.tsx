  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      if (data.discovered_count > 0) {
        ...
        const { successCount, totalAttempted } = await uploadMultipleDatasheets(newComponents);
  ...
  const handleImportExcel = async (
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/projects/${projectId}/components/upload`,
        {
          method: "POST",
          body: formData,
        }
