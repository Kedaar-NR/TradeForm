...
const ComponentDetailDrawer: React.FC<ComponentDetailDrawerProps> = ({
  ...
  const averageScore = useMemo(() => {
    const numericScores = scores
      .map((score) => (typeof score.score === "number" ? score.score : null))
  ...
  const averageConfidence = useMemo(() => {
    const numericConfidence = scores
      .map((score) =>
        typeof score.extractionConfidence === "number"
          ? Math.min(Math.max(score.extractionConfidence, 0), 1)
          : null
      )
      .filter((value): value is number => value !== null);
    if (!numericConfidence.length) {
      return null;
    }
    const total = numericConfidence.reduce((sum, value) => sum + value, 0);
    return total / numericConfidence.length;
  }, [scores]);
  ...
  return (
    <div>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 md:p-8 pointer-events-none">
        <div
          className="w-full max-w-4xl h-full max-h-[90vh] bg-white shadow-2xl rounded-lg flex flex-col animate-fade-in pointer-events-auto"
                ...
                {isLoadingScores ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Define evaluation criteria for this project to see
                      component scores.
                    </p>
                  </div>
                ) : scores.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      This component hasn't been scored yet. Use the "Score All
                      Components" button to generate AI scores.
                    </p>
                  </div>
                ) : (
                    ...
                    <div className="space-y-4">
                      {criteria.map((criterion) => {
                        ...
                        const normalizedConfidence =
                          ...
                          typeof score?.extractionConfidence === "number"
                            ? Math.min(
                                Math.max(score.extractionConfidence, 0),
                                1
                              )
                            : null;
